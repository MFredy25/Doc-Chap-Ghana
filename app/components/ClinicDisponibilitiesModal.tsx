...
export default function ClinicDisponibilitiesModal({
  open,
  clinicId,
  clinicName,
  consultationModes,
  ...
  useEffect(() => {
    ...
    if (!firestore || !clinicId) {
      setError(
        "Unable to load consultation hours. Firebase or clinic information is missing."
      );
      setLoading(false);
      return;
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshotError) => {
        console.error(
          "[ClinicDisponibilitiesModal] Realtime error:",
          snapshotError
        );
  ...
  async function saveHours() {
    ...
    if (!firestore || !clinicId) {
      setError(
        "Unable to save consultation hours. Firebase or clinic information is missing."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "[ClinicDisponibilitiesModal] Save error:",
        saveError
      );
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="clinic-disponibilities-title"
      onMouseDown={(event) => {
        if (
        ...
        <div className="relative overflow-hidden border-b border-blue-900/20 bg-gradient-to-br from-[#071b3a] via-[#0b2d5f] to-[#164a8a] px-5 py-5 text-white sm:px-6">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              ...
              <h2
                id="clinic-disponibilities-title"
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-blue-100">
                {clinicName
                  ? `Set the weekly consultation hours for ${clinicName}.`
