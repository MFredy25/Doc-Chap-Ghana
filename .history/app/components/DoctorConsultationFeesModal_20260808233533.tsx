...
export default function DoctorConsultationFeesModal({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const pricing = data.consultationPricing ?? {};
        ...
        setFees({
          ...
          video: safeAmount(pricing.video?.amount, 0),
          phone: safeAmount(pricing.phone?.amount, 0),
        });
  ...
  async function saveFees() {
    ...
    const inPerson = Number(fees.inPerson);
    const video = Number(fees.video);
    const phone = Number(fees.phone);

    const values = [
      {
        enabled: consultationModes.inPersonEnabled,
        label: "In-person consultation",
        value: inPerson,
      },
      {
          ...
          consultationPricing: {
            ...
            inPerson: {
              ...
              amount:
                consultationModes.inPersonEnabled
                  ? inPerson
                  : existingPricing.inPerson?.amount ?? 0,
            },
            video: {
              enabled:
                consultationModes.teleconsultationEnabled,
              amount:
                consultationModes.teleconsultationEnabled
                  ? video
                  : existingPricing.video?.amount ?? 0,
            },
            phone: {
              enabled:
                consultationModes.phoneConsultationEnabled,
              amount:
                consultationModes.phoneConsultationEnabled
                  ? phone
                  : existingPricing.phone?.amount ?? 0,
            },
                ...
                return (
                  ...
                  >
                    ...
                    <label className="mt-4 block">
                      ...
                      <div className="mt-2 flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-500/10 dark:border-zinc-800 dark:bg-zinc-900">
                        ...
                        <input
                          ...
                          className="h-14 w-full min-w-0 bg-transparent text-base font-bold text-zinc-950 outline-none dark:text-white"
                        />
