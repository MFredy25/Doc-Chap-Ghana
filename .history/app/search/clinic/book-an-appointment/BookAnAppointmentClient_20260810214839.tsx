...
export default function BookAnAppointmentClient() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(
      ...
      async (currentUser) => {
        if (!currentUser) {
          const next =
            "/search/clinic/book-an-appointment?id=" +
            encodeURIComponent(
              providerId
            );
  ...
  useEffect(() => {
    ...
    async function loadAvailability() {
      ...
      try {
        ...
        const response =
          await fetch(
            "/api/clinic-book-an-appointment?" +
              params.toString(),
            {
              cache:
                "no-store",
        ...
        setSelectedSpecialtyId(
          (current) =>
            loadedSpecialties.some(
              (item) =>
                item.id === current
            )
              ? current
              : loadedSpecialties[0]?.id ||
                ""
        );
  ...
  useEffect(() => {
    ...
    const currentStillAllowed =
      appointmentType === "in_person"
        ? selectedSpecialty.pricing.inPerson.enabled &&
          provider?.modes.inPerson
        : appointmentType === "teleconsultation"
        ? selectedSpecialty.pricing.video.enabled &&
          provider?.modes.teleconsultation
        : appointmentType === "phone"
        ? selectedSpecialty.pricing.phone.enabled &&
  ...
  const consultationPrice =
    selectedPricing?.enabled
      ? selectedPricing.amount
      : 0;

  const consultationCurrency =
    selectedSpecialty?.currency ||
    "GHS";
  ...
  async function confirmAppointment() {
    if (
      ...
      setError(
        "Choose a specialty, consultation type and an available time."
      );
      return;
    }
    ...
    try {
      ...
      const confirmation = {
        ...
        consultationPrice:
          payload.appointment.consultationPrice ??
          consultationPrice,
        consultationCurrency:
          payload.appointment.consultationCurrency ||
          consultationCurrency,
        date:
      ...
      /* --------------------------------------------------------
         APPOINTMENT EMAILS
         Patient + clinic only. No administrator email here.
      -------------------------------------------------------- */

      try {
        const emailResponse =
          await fetch(
            "/api/send-email-clinic-new-appointment",
            {
              method:
                "POST",
        ...
        if (
          ...
          console.warn(
            "[ClinicBooking] Appointment email notification failed:",
            emailPayload
          );
        }
      } catch (
         ...
         * Email delivery must never cancel or hide the booking.
         */
        console.warn(
          "[ClinicBooking] Appointment email notification error:",
          emailError
        );
      }
      ...
      router.push(
        "/search/clinic/book-an-appointment/appointment-confirmed"
      );
    } catch (submitError) {
      setError(
  ...
  return (
    <div className="min-h-screen bg-[#f7faf9] dark:bg-black">
      ...
      <main>
        <section className="bg-gradient-to-br from-[#063b34] via-[#08745e] to-[#10a37f] text-white">
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
              <div className="space-y-6">
                ...
                <section className="rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-zinc-500">
                        Select the clinic specialty for this appointment.
                      </p>
                    </div>
                  ...
                  {specialties.length === 0 ? (
                    <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
                      This clinic has not added any available specialty yet.
                    </div>
                  ) : (
                            ...
                            <button
                              ...
                              className={`rounded-2xl border p-4 text-left transition ${
                                selected
                                  ...
                                  : "border-zinc-200 bg-zinc-50 hover:border-emerald-300 dark:border-zinc-800 dark:bg-zinc-900"
                              }`}
                            >
                ...
                <section className="rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                  ...
                  <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {provider.modes.inPerson && selectedSpecialty?.pricing.inPerson.enabled && (
                      <button
                        type="button"
                        onClick={() =>
                          setAppointmentType(
                      ...
                      >
                        ...
                        <div className="mt-1 text-xs font-bold text-cyan-700">
                          {selectedSpecialty
                            ? formatPrice(
                                selectedSpecialty.pricing.inPerson.amount,
                                selectedSpecialty.currency
                              )
                            : ""}
                        </div>
                    ...
                    {provider.modes.teleconsultation && selectedSpecialty?.pricing.video.enabled && (
                      <button
                        type="button"
                        onClick={() =>
                          setAppointmentType(
                      ...
                      >
                        ...
                        <div className="mt-1 text-xs font-bold text-violet-700">
                          {selectedSpecialty
                            ? formatPrice(
                                selectedSpecialty.pricing.video.amount,
                                selectedSpecialty.currency
                              )
                            : ""}
                        </div>
                    ...
                    {provider.modes.phone && selectedSpecialty?.pricing.phone.enabled && (
                      <button
                        type="button"
                        onClick={() =>
                          setAppointmentType(
                      ...
                      >
                        ...
                        <div className="mt-1 text-xs font-bold text-emerald-700">
                          {selectedSpecialty
                            ? formatPrice(
                                selectedSpecialty.pricing.phone.amount,
                                selectedSpecialty.currency
                              )
                            : ""}
                        </div>
              ...
              <aside className="space-y-5">
                ...
                <section className="rounded-[28px] border border-emerald-200 bg-emerald-50/70 p-5 shadow-sm dark:border-emerald-900/40 dark:bg-emerald-950/20">
                  ...
                  <div className="mt-5 border-t border-emerald-200 pt-4 dark:border-emerald-900/40">
                    <div className="text-xs font-semibold text-zinc-500">
                      Price
                    </div>
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-4 text-sm font-black text-white disabled:opacity-60"
                >
                  {submitting ? (
                    <>
