...
async function loadBusy(
  ...
  const snapshot =
    await adminDb
      .collection(
        `clinics/${providerId}/appointments`
      )
      .get();
...
async function getProvider(
  ...
  if (
    ...
    const configSnapshot =
      await adminDb
        .doc(`clinics/${id}/configuration/general`)
        .get();
    ...
    const clinic = asObject(data.clinic);
    ...
    const openingHours =
      asObject(data.openingHours);

    const nestedOpeningHours =
      asObject(availability.openingHours);

    const weekCandidates = [
      asObject(availability.week),
      asObject(openingHours.week),
      asObject(nestedOpeningHours.week),
    ];
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "This clinic is not currently accepting appointment requests.",
        },
        {
          status: 404,
        }
...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Please choose a valid clinic specialty.",
        },
        {
          status: 400,
        }
    ...
    if (
      ...
      !provider.acceptsNewPatients ||
      !modeAllowed ||
      !specialtyPricing.enabled ||
      specialtyPricing.amount <= 0
    ) {
      return NextResponse.json(
        {
          ok: false,
    ...
    const providerRef =
      adminDb.doc(
        `clinics/${providerId}/appointments/${appointmentId}`
      );
    ...
    const lockRef =
      adminDb.doc(
        `clinics/${providerId}/appointment_slot_locks/${lockId}`
      );
